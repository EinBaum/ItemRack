...
ItemRack.SetnameBlacklist = {
	...
	["Nothing"] = true, -- canonical empty-durability-slots set, re-asserted in InitCore
}
...
function ItemRackOpt.OnLoad(self)
	...
	-- drives the scrollable options list; built here because the XML buttons must exist
	-- depend = only active while optset[depend]=="ON"; combatlock = greyed out in combat
	ItemRackOpt.OptInfo = {
		...
		{type="check",optset=ItemRackSettings,variable="MenuHorizontal",label="Horizontal menu",tooltip="Lay out the popup menu horizontally instead of vertically."},
	...
	ItemRackOpt.TabOnClick(self,1) -- tab 1 = config
...
function ItemRackOpt.InitializeSliders()
	for i=1,#(ItemRackOpt.OptInfo) do
		local opt = ItemRackOpt.OptInfo[i]
...
function ItemRackOpt.UpdateInv()
	...
	for i=0,19 do
		local icon = _G["ItemRackOptInv"..i.."Icon"]
		local border = _G["ItemRackOptInv"..i.."Border"]
		border:Hide()
		local texture
		if ItemRackOpt.Inv[i].id~=0 then
			texture = select(2,ItemRack.GetInfoByID(ItemRackOpt.Inv[i].id))
			...
			texture = select(2,GetInventorySlotInfo(ItemRack.SlotInfo[i].name))
		end
		icon:SetTexture(texture)
		local item = _G["ItemRackOptInv"..i]
...
function ItemRackOpt.SetsIconScrollFrameUpdate()
	...
	for i=1,25 do
		local item = _G["ItemRackOptSetsIcon"..i]
		local idx = (offset*5) + i
		if idx<=#(ItemRackOpt.Icons) then
			local texture = ItemRackOpt.Icons[idx]
...
function ItemRackOpt.SaveSet()
	...
	if ItemRack.SetnameBlacklist[setname] then
		return
	end
...
function ItemRackOpt.ShowCloakHelm()
	local setname = ItemRackOptSetsName:GetText()
	local set = ItemRackUser.Sets[setname]
	if setname ~= ItemRackUser.CurrentSet or not set then
		return
	end
...
function ItemRackOpt.PickSetOnHide()
	...
	if ItemRackOpt.EventSelected and ItemRackOpt.prevFrame==ItemRackOptSubFrame3 then
		...
		-- an enabled event with no set can't stay enabled
...
function ItemRackOpt.SetListScrollFrameUpdate()
	...
	for i=1,10 do
		local item = _G["ItemRackOptSetList"..i]
		local idx = offset + i
...
function ItemRackOpt.PopulateSetList()
	wipe(ItemRackOpt.SetList)
	for i in pairs(ItemRackUser.Sets) do
		if not string.match(i,"^~") then -- internal sets stay out of the list
...
function ItemRackOpt.ListScrollFrameUpdate()
	...
	for i=1,11 do
		local button = _G["ItemRackOptList"..i]
		...
		if idx<=#(ItemRackOpt.OptInfo) then
			...
			local lock = ItemRack.inCombat and opt.combatlock
...
function ItemRackOpt.OptListCheckButtonOnClick(self,override)
	...
	elseif opt.variable=="ShowMinimap" then
		opt.optset["minimap"]["hide"] = check ~= "ON"
	...
	elseif opt.variable=="AlwaysShowOnyxiaCloak" then
...
function ItemRackOpt.BindFrameOnKeyDown(self,key)
	...
	if keyPressed then
		...
		if oldAction~="" and keyPressed~=ItemRackOpt.Binding.currentKey then
			...
			ItemRackOptBindFrame:EnableKeyboard(false)
			ItemRackOptBindFrame:EnableMouse(false)
...
function ItemRackOpt.PopulateSortList(slot)
	...
	-- collapse stop-queue duplicates left in saved queues
	local kept
	for i = #sortList, 1, -1 do
	...
	ItemRack.BuildMenu(slot,1) -- dummy menu to fetch all wearable items for the slot
	ItemRackMenuFrame:Hide()
	for i=1,#(ItemRack.Menu) do
		ItemRackOpt.AddToSortList(sortList,ItemRack.Menu[i])
	end
	-- BuildMenu only seeds 0 when the slot is worn; the stop marker must be placeable even when the slot is empty
	if ItemRackSettings.AllowEmpty == "ON" then
		ItemRackOpt.AddToSortList(sortList, 0)
	end
...
function ItemRackOpt.AddToSortList(sortList,id)
	-- 0 is the stop-queue sentinel; UpdateIRString isn't meaningful for it, so dedup by literal equality
	if id == 0 then
		for i = 1, #sortList do
			if sortList[i] == 0 then return end
		end
		table.insert(sortList, 0)
		return
	end
	-- level/spec-normalized compare; enchant, gems and suffix still distinguish entries
	local normalized = ItemRack.UpdateIRString(id)
	for i=1,#(sortList) do
		if sortList[i] ~= 0 and ItemRack.UpdateIRString(sortList[i]) == normalized then
			return
		end
	end
...
function ItemRackOpt.SortListScrollFrameUpdate()
	...
	for i=1,11 do
		local item = _G["ItemRackOptSortList"..i]
		local idx = offset + i
		if sortList and idx<=#(sortList) then
			local name, texture, _, quality
...
function ItemRackOpt.ValidateSortButtons()
	...
	if not selected or #(list)<2 then
	...
	-- keep the selection visible, moving the thumb as needed, unless shift is held
	if not IsShiftKeyDown() and selected then
...
function ItemRackOpt.PopulateEventList()
	...
	for i in pairs(events) do
		...
		elseif events[i].Type=="Script" and user.Enabled[i] then
			setname = "zzz" -- fake set name so enabled scripts sort with set-defined events
		end
	...
	-- set-defined events first, each group alphabetical
...
function ItemRackOpt.EventListScrollFrameUpdate()
	...
	for i=1,9 do
		local item = _G["ItemRackOptEventList"..i]
		local idx = offset + i
		if idx<=#(list) then
